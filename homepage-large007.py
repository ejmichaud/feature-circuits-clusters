...
# cluster_files = os.listdir("data-large007")
...
# with open(f"data-large007/{cluster_file}") as f:
#     clusters = json.load(f)

# def get_clusters():
#     if 'clusters' not in st.session_state:
#         with open("data-large007/cluster_is.pkl", "rb") as f:
...
#         with gzip.open("data-large007/contexts-pythia-70m-100k.pkl.gz", "rb") as f:
...
def get_idxs():
    if 'idxs' not in st.session_state:
        with open("data-large007/idxs.pkl", "rb") as f:
...
#         st.session_state['losses'] = np.load("data-large007/loss_curves.npy")
...
def get_mean_loss():
    if 'mean_loss' not in st.session_state:
        st.session_state['mean_loss'] = np.load("data-large007/mean_loss_curve.npy")
...
#         with gzip.open("data-large007/permuted_Cs.pkl.gz", "rb") as f:
...
#         with open("data-large007/unpermuted_Cs.pkl", "rb") as f:
...
st.sidebar.write("These are clusters for pythia-70m, with a loss threshold of 0.1 nats, across 100k tokens, using k-means for clustering. I projected the gradients (~19m-dimensional) onto a 30k-dimensional subspace for clustering. There are 4000 clusters in total.")
...
#         samples = json.load(f)
# if 'samples' not in st.session_state:
#     with open("data-large007/contexts-pythia-70m-100k.json") as f:
#         # samples = json.load(f)
...
with SqliteDict("data-large007/database.sqlite") as db:
